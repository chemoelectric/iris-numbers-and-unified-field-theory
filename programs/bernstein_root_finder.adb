...
procedure bernstein_root_finder is
   ...
   with
     pre  => coeffs'length >= 1 and u >= 0.0 and u <= 1.0,
   ...
   with pre => coeffs'length >= 1, post => compute_error_filter'result >= 0.0
   is
   ...
   procedure compute_derivative (coeffs : float_array; deriv : out float_array)
   ...
   with
     pre =>
       ...
       and domain_b > domain_a
       and tol > 0.0
       and roots'length >= coeffs'length - 1
   is
      m            : constant integer := coeffs'length - 1;
      eps_filter   : real;
      b0           : real;
...
begin
   ...
   while idx_r < n_found loop
      ...
      ada.float_text_io.put
        (item => float (r_val), fore => 1, aft => 8, exp => 0);
      ada.text_io.new_line;
      idx_r := idx_r + 1;
   end loop;
